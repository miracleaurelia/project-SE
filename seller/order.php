...
if (isset($_POST['form1'])) {
    ...
    if ($valid == 1) {
        ...
        $admin_email = 'miracleaureliarec@gmail.com';

        $order_detail = '';
        $statement = $pdo->prepare("SELECT tc.cust_id,tc.cust_name,tc.cust_email,
        tpr.p_id,tpr.p_name,t_o.quantity,tpr.p_current_price *t_o.quantity as 'Total Price',
        tp.payment_id,tp.payment_method,tp.payment_status,tp.payment_date,tp.shipping_status,
        t_o.product_name,t_o.size,t_o.color,tpr.p_current_price as 'unit_price'
        ...
        foreach ($result as $row) {
            if ($row['payment_method'] == 'PayPal') :
                $payment_details = '
            Transaction Id: ' . $row['payment_id'] . '<br>
            ';
            ...
            Customer Email: ' . $row['cust_email'] . '<br>
            Payment Method: ' . $row['payment_method'] . '<br>
            Payment Date: ' . $row['payment_date'] . '<br>
            Paid Amount: ' . $row['Total Price'] . '<br>
        ...
        ON tc.cust_id = tp.customer_id JOIN tbl_product tpr 
        ON tpr.p_id = t_o.product_id
        WHERE tp.payment_id = ? AND tpr.SellerID = ? 
        GROUP BY tp.payment_id
        ORDER BY tp.payment_id DESC");
        $statement->execute(array($_POST['payment_id'], $_SESSION['seller']['id']));
        $result = $statement->fetchAll(PDO::FETCH_ASSOC);
        foreach ($result as $row) {
            $i++;
            $order_detail .= '
                            ...
                            ?>
                                <tr class="<?php if ($row['payment_status'] == 'Pending') {
                                    ...
                                    <td>
                                        ...
                                        <b>Email:</b><br> <?php echo $row['cust_email']; ?><br><br>
                                    ...
                                    <td>
                                        <?php
                                        $statement1 = $pdo->prepare("SELECT tc.cust_id,tc.cust_name,tc.cust_email,
                                        tpr.p_id,tpr.p_name,t_o.quantity,tpr.p_current_price *t_o.quantity as 'Total Price',
                                        tp.payment_id,tp.payment_method,tp.payment_status,tp.payment_date,tp.shipping_status,
                                        t_o.product_name,t_o.size,t_o.color,tpr.p_current_price as 'unit_price'
                                        FROM `tbl_payment` tp JOIN tbl_order t_o
                                        ...
                                        GROUP BY tp.payment_id
                                        ORDER BY tp.payment_id DESC");
                                        $statement1->execute(array($_SESSION['seller']['id']));
                                    ...
                                    <td>Rp.<?php echo $row['Total Price']; ?></td>
                                    <td>
                                        ...
                                        <!-- <?php
                                                ...
                                                ?>
                                            <a href="./order-change-status.php?id=<?php echo $row['payment_id']; ?>&task=Completed" class="btn btn-success btn-xs" style="width:100%;margin-bottom:4px;">Mark Complete</a>
                                        <?php
                                                }
                                        ?> -->
                                    </td>
                                    <td>
                                        ...
                                        ?>
                                                <a href="./shipping-change-status.php?id=<?php echo $row['payment_id']; ?>&task=Completed" class="btn btn-warning btn-xs" style="width:100%;margin-bottom:4px;">Mark Complete</a>
                                        <?php
                                            }
                                        }
                                        ?>
                                    ...
                                    <td>
                                        <a href="#" class="btn btn-danger btn-xs" data-href="order-delete.php?id=<?php echo $row['payment_id']; ?>" data-toggle="modal" data-target="#confirm-delete" style="width:100%;">Delete</a>
                                    </td>
                                </tr>
                            <?php
                            }
